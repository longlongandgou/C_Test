...
#include"/storage/emulated/0/练习/我的文档/map.h"
...
int playMap(char m[8][8])
{
	...
	for (i=0; i<8; i++)
	{
		for (j=0; j<8; j++)
		{
			...
			if (m[i][j] == 1)//人物
			{
				printf("\033[47;30m人\033[0m");
			}
			if (m[i][j] == 2)//墙壁
			{
				printf("\033[41;37m围\033[0m");
			}
			if (m[i][j] == 3)//箱子
			{
				printf("\033[43;37m囗\033[0m");
			}
			if (m[i][j] == 4)//终点
			{
				printf("\033[42;30m叵\033[0m");
			}
			if (m[i][j] == 5)//终点+箱子
			{
				printf("\033[43;32m回\033[0m");
			}
			if (m[i][j] == 6)//终点+人物
			{
				printf("\033[43;36m区\033[0m");
			}
